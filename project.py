...
from db.db_access import load_session, Lexemes, Sources, Users
...
def homepage():
    ...
    if 'username' in login_session:
        paragraph = ['Welcome to the IELex2 website!','You are logged in as: %s' % login_session['username']]
    else:
        paragraph = ['Welcome to the IELex2 website!','You can log-in from this page.']
...
@app.route('/lexemes/')
def lexemes_menu():
    lexemes = session.query(Lexemes).all()
    #Make sure only logged in users can access editable lexemes template
    if 'username' not in login_session:
        return render_template('lexemes_public.html', lexemes=lexemes)
    else:
        return render_template('lexemes.html', lexemes=lexemes)


@app.route('/lexeme/new/', methods=['GET', 'POST'])
def new_lexeme_item():
    lexemes = session.query(Lexemes).all()

    if 'username' not in login_session:
        flash('Only logged in users can create lexemes!')
        return redirect(url_for('homepage'))
    
    if request.method=='POST':
        new_lex_entry = Lexemes(
                          lexeme = request.form['lex'],
                          meaning = request.form['meaning'],
                          cognate_id = request.form['cognate_id'],
                          cognate_sources = request.form['cognate_sources'],
                          iso_lang_code = request.form['iso_lang_code'],
                          lexeme_sources = request.form['lexeme_sources'],
                          user_id = login_session['user_id']
                          )
        session.add(new_lex_entry)
        session.commit()
        flash('New lexeme created!')        
        return render_template('lexemes.html', lexemes=lexemes)
    else:
...
@app.route('/lexemes/<lex_trg_id>_<lex_trg>/delete/', methods=['GET', 'POST'])
def delete_lexeme_item(lex_trg, lex_trg_id):
    lexeme = session.query(Lexemes).filter_by(id = lex_trg_id).one()

    if 'username' not in login_session:
        flash('Only logged in users can do that!')
        return redirect('lexemes_menu')
    
    creator = get_user_info(lexeme.user_id)

    #NB. Could restrict access even further, by ensuring only users who created an item can delete it
    '''
    creator = get_user_info(lexeme.user_id)
    lex = session.query(Lexemes).filter_by(id = lex_trg_id).one()
    if lexeme.user_id!=login_session['user_id']:
        flash('Only the creator of a lexeme can delete it!')
        return redirect('lexemes_menu')
        #Alt: return "<script>function myFunction(){alert('Only the creator of a lexeme can delete it!');}</script><body onload='myFunction()''>"
    else:
        if request.method=='POST':
            session.delete(lexeme)
            session.commit()
            flash('Deleted: '+str(lexeme.lexeme))
            return redirect(url_for('lexemes_menu'))
    '''
    ...
    if request.method=='POST':
        ...
        return render_template('delete_lexeme.html', lex=lexeme.lexeme, lex_id=lexeme.id, creator=creator)


@app.route('/lexemes/<lex_trg_id>_<lex_trg>/edit/', methods=['GET', 'POST'])
def edit_lexeme_item(lex_trg, lex_trg_id):
    lexeme = session.query(Lexemes).filter_by(id = lex_trg_id).one()

    if 'username' not in login_session:
        flash('Only logged in users can edit lexemes!')
        return redirect('lexemes_menu')
    ...
    if request.method=='POST':
        if request.form['text']:
            lexeme.lexeme = request.form['lex'],
            lexeme.meaning = request.form['meaning'],
            lexeme.cognate_id = request.form['cognate_id'],
            lexeme.cognate_sources = request.form['cognate_sources'],
            lexeme.iso_lang_code = request.form['iso_lang_code'],
            lexeme.lexeme_sources = request.form['lexeme_sources'],
            lexeme.user_id = login_session['user_id']
        session.add(lexeme)
        session.commit()
        flash('Lexeme edited!')
        return redirect(url_for('lexemes_menu'))
        ...
        return render_template('edit_lexeme.html', lex=lexeme, creator=creator['name'])
